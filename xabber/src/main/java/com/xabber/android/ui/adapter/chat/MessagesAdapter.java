...
public class MessagesAdapter extends RealmRecyclerViewAdapter<MessageItem, BasicMessageVH>
    ...
    //发红包
    public static final int VIEW_TYPE_OUTING_RED_ENVELOPE = 7;
    //收红包
    public static final int VIEW_TYPE_INCOMING_RED_ENVELOPE = 8;
    //领取红包
    public static final int VIEW_TYPE_RECEIVE_RED_ENVELOPE = 9;
    //消息撤回
    public static final int VIEW_TYPE_IMESSAGE_REVOKE = 10;
    ...
    public int getItemViewType(int position) {
        ...
        if (messageItem.getMessage_type() == MessageItem.MessageType.REVOKE)
            return VIEW_TYPE_IMESSAGE_REVOKE;

        if (messageItem.getMessage_type() == MessageItem.MessageType.RECEIVE_RED_ENVELOPE) {
            return VIEW_TYPE_RECEIVE_RED_ENVELOPE;
        }

        if (messageItem.isIncoming()) {
            if (messageItem.getMessage_type() == MessageItem.MessageType.REDENVELOPE)
                return VIEW_TYPE_INCOMING_RED_ENVELOPE;
            ...
            if (messageItem.getMessage_type() == MessageItem.MessageType.REDENVELOPE)
                return VIEW_TYPE_OUTING_RED_ENVELOPE;
            return noFlex ? VIEW_TYPE_OUTGOING_MESSAGE_NOFLEX : VIEW_TYPE_OUTGOING_MESSAGE;
        }
    }
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_OUTGOING_MESSAGE_NOFLEX:
                return new NoFlexOutgoingMsgVH(LayoutInflater.from(parent.getContext())
                        ...
                        this, this, this, appearanceStyle);
            case VIEW_TYPE_INCOMING_RED_ENVELOPE:
                return new RedEnvelopeIncomingMsgVH(LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.item_message_incoming, parent, false),
                        this, this, this, appearanceStyle);

            case VIEW_TYPE_OUTING_RED_ENVELOPE:
                return new RedEnvelopeOutgoingMessageVH(LayoutInflater.from(parent.getContext())
                        .inflate(R.layout.item_message_outgoing, parent, false),
                        this, this, this, appearanceStyle);
            case VIEW_TYPE_IMESSAGE_REVOKE:
                return new RevokeMsgVH(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_message_revoke, parent, false));
            case VIEW_TYPE_RECEIVE_RED_ENVELOPE:
                return new ReceiveRedEnvelopeMsgVH(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_message_revoke, parent, false));
            default:
                return null;
        }
    }
    ...
    public void onBindViewHolder(final BasicMessageVH holder, int position) {
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_OUTGOING_MESSAGE_NOFLEX:
                ...
                break;
            case VIEW_TYPE_INCOMING_RED_ENVELOPE:
                ((RedEnvelopeIncomingMsgVH) holder).bind(messageItem, extraData);
                break;
            case VIEW_TYPE_OUTING_RED_ENVELOPE:
                ((RedEnvelopeOutgoingMessageVH) holder).bind(messageItem, extraData);
                break;
            case VIEW_TYPE_IMESSAGE_REVOKE:
                ((RevokeMsgVH) holder).setUser(!messageItem.isIncoming());
                break;
            case VIEW_TYPE_RECEIVE_RED_ENVELOPE:
                ((ReceiveRedEnvelopeMsgVH) holder).setMessage(messageItem.isIncoming() ? userName + "领取了红包" : "您领取了红包");
                break;
        }
    ...
    @Override
    public void onRedEnvelopeClick(int messagePosition, int redEnvelopePosition, String messageUID) {
        Realm realm = MessageDatabaseManager.getInstance().getRealmUiThread();
        MessageItem messageItem = realm.where(MessageItem.class).equalTo(MessageItem.Fields.UNIQUE_ID, messageUID).findFirst();
        if (messageItem != null) {
            RealmList<RedEnvelope> redenvelopes = messageItem.getRedenvelopes();
            RedEnvelope redEnvelope = redenvelopes.get(redEnvelopePosition);
            if (redEnvelope.isRedenvelope_receive()) {

                return;
            }
            View view = LayoutInflater.from(context).inflate(R.layout.dialog_recevice_red_envelope, null, false);
            TextView tvMoney = view.findViewById(R.id.tv_money);
            tvMoney.setText(redEnvelope.getRedenvelope_money() + "元");
            new AlertDialog.Builder(context).setView(view).create().show();
            boolean self = !messageItem.isIncoming();
            MessageManager.getInstance().sendReceviceRedEnvelopeMessage(account, user, "领取了红包", messageItem.getStanzaId(), self);
