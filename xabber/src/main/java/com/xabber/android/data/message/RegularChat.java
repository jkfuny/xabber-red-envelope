...
public class RegularChat extends AbstractChat {
    ...
    @Override
    protected MessageItem createRedEnvelopeMessageItem(String money, String remarks) {
        ...
        MessageItem messageItem = createNewMessageItem("红包[" + remarks + "]");
...
//            }
            ...
            Log.i(" 收到的 XEP-0221", message.toXML().toString());
            ...
            ExtendedFormField.OldMessageAction oldMessageAction = OldMessageManager.parseOldMessageAction(packet);

            RealmList<Attachment> attachments = HttpFileUploadManager.parseFileMessage(packet);

            RealmList<RedEnvelope> redEnvelopes = RedEnvelopeMananger.parseRedEnvelopeMessage(packet);
            ...
            if (oldMessageAction != null) {
                doOldMessageAction(oldMessageAction);
                //领取红包
                if (oldMessageAction.getAction() == MessageItem.MessageType.RECEIVE_RED_ENVELOPE) {
                    createAndSaveReceviceRedEnvelopeMessage(true, uid, resource, text, null, getDelayStamp(message),
                            true, true, encrypted,
            ...
            else if (redEnvelopes != null && redEnvelopes.size() > 0) {
                createAndRedEnvelopeNewMessage(true, uid, resource, text, null, getDelayStamp(message),
                        true, true, encrypted,
                        isOfflineMessage(account.getFullJid().getDomain(), packet),
                        packet.getStanzaId(), originalStanza, null,
                        originalFrom, forwardIds, false, false, redEnvelopes);
