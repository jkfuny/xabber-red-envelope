...
public class CustomDataProvider extends ExtensionElementProvider<DataForm> {
                ...
                case XmlPullParser.START_TAG:
                    ...
                    switch (name) {
                        ...
                        case ExtendedFormField.Media.ELEMENT:
                            ...
                            break;
                            //红包解析
                        case ExtendedFormField.RedEnvelopeType.ELEMENT:
                                formField.setRedEnvelopeType(parseRedenvelope(parser));
                            break ;
                            //撤回或领红包等旧数据操作解析
                        case ExtendedFormField.OldMessageAction.ELEMENT:
                            formField.setOldMessageAction(parseOldMessageAction(parser));
                            break ;
    ...
    protected static ExtendedFormField.RedEnvelopeType parseRedenvelope(XmlPullParser parser) throws XmlPullParserException, IOException {
        final String type = parser.getAttributeValue("", "type");
        final String remarks = parser.getAttributeValue("", "remarks");
        final String money = parser.getAttributeValue("", "money");

        parser.next();
        return new ExtendedFormField.RedEnvelopeType(type, remarks, money);
    }
    protected static ExtendedFormField.OldMessageAction parseOldMessageAction(XmlPullParser parser) throws XmlPullParserException, IOException {
        final String messagteId = parser.getAttributeValue("", "messagteId");
        final String action = parser.getAttributeValue("", "action");

        parser.next();
        return new ExtendedFormField.OldMessageAction(messagteId, TextUtils.isEmpty(action)? MessageItem.MessageType.REVOKE :Integer.parseInt(action));
    }
