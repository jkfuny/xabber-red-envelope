...
public class MessageManager implements OnLoadListener, OnPacketListener, OnDisconnectListener,
    ...
    public void sendRedEnvelopeMessage(AccountJid account, UserJid user, final String money, final String remarks) {
        final AbstractChat chat = getOrCreateChat(account, user);
        final long startTime = System.currentTimeMillis();
        ...
        MessageDatabaseManager.getInstance().getRealmUiThread()
                .executeTransactionAsync(new Realm.Transaction() {
                    @Override
                    public void execute(Realm realm) {
                        MessageItem newMessageItem = chat.createRedEnvelopeMessageItem(money, remarks);
    ...
    public void sendReceviceRedEnvelopeMessage(AccountJid account, UserJid user, final String text, String stanzaId, boolean self) {
        final AbstractChat chat = getOrCreateChat(account, user);
        final MessageItem newMessageItem = chat.createNewMessageItem(text);
        chat.saveReceiveRedMessage(newMessageItem,self, stanzaId);

    }


    public void sendRevokeMessage(AccountJid account, UserJid user, final String messageId) {
        AbstractChat chat = getOrCreateChat(account, user);
        chat.sendReVokeMessage(messageId);
    }
